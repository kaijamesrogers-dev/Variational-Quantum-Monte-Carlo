"""
VMC for the hydrogen molecule (H2) in 3D.

Extends the single-proton VMC method to two electrons and two fixed
protons, using the Born-Oppenheimer approximation. The trial
wavefunction is a Slater-Jastrow form with three variational
parameters (theta1, theta2, theta3): the Slater part allows either
electron to sit near either proton, and the Jastrow factor adds
explicit electron-electron correlation.

As in the hydrogen-atom case, theta1-3 are unknown and must be found
by simulated annealing: for any fixed (theta1, theta2, theta3) the
wavefunction is fully defined, its energy is estimated via Monte
Carlo, and the annealing search moves towards the parameter values
that minimise this energy.

Repeating this minimisation across a range of proton separations gives
an energy-vs-bond-length curve, which is fitted to a Morse potential
to extract the predicted equilibrium bond length and dissociation
energy (Fig. 4 in the report). A 2D histogram of sampled electron
positions (Fig. 5) visualises the resulting bonding orbital.
"""

import time

import numpy as np
import matplotlib.pyplot as plt
from scipy.optimize import curve_fit

# Match text to lab report
plt.rcParams.update({
    "font.size": 14,
    "axes.titlesize": 14,
    "axes.labelsize": 14,
    "ytick.labelsize": 14,
    "legend.fontsize": 14,
    "font.family": "serif",
    "mathtext.fontset": "cm",
    "axes.unicode_minus": False,
})

THETA_INITIAL = 0.5
E_SINGLE = -0.5  # ground-state energy of an isolated hydrogen atom, used in the Morse fit


def psi_h2(x_a, y_a, z_a, x_b, y_b, z_b, theta1, theta2, theta3, q1, q2):
    """
    Slater-Jastrow trial wavefunction psi(r1, r2; theta1, theta2, theta3)
    for the H2 molecule (Eq. 15).
    """
    r1 = np.stack((x_a, y_a, z_a), axis=-1)
    r2 = np.stack((x_b, y_b, z_b), axis=-1)

    q1 = np.asarray(q1)
    q2 = np.asarray(q2)

    # distances to nuclei
    r1_q1 = np.linalg.norm(r1 - q1, axis=-1)
    r1_q2 = np.linalg.norm(r1 - q2, axis=-1)
    r2_q1 = np.linalg.norm(r2 - q1, axis=-1)
    r2_q2 = np.linalg.norm(r2 - q2, axis=-1)

    # electron-electron distance
    r12 = np.linalg.norm(r1 - r2, axis=-1)

    # Slater part: symmetrised over which electron sits near which proton
    slater = np.exp(-theta1 * (r1_q1 + r2_q2)) + np.exp(-theta1 * (r1_q2 + r2_q1))

    # Jastrow correlation factor
    jastrow = np.exp(-theta2 / (1.0 + theta3 * r12))

    return slater * jastrow


def pdf_xyz(x_a, y_a, z_a, x_b, y_b, z_b, theta1, theta2, theta3, q1, q2):
    """Probability density |psi|^2 for the H2 trial wavefunction."""
    psi = psi_h2(x_a, y_a, z_a, x_b, y_b, z_b, theta1, theta2, theta3, q1, q2)
    return psi**2


def metropolis_2protons_multi(step_size, pdf, n_walkers, theta1, theta2, theta3,
                               q_1, q_2, iterations=100):
    """
    6D Metropolis sampler (both electrons move independently) with
    multiple walkers evolved in parallel. The first 20% of each chain
    is discarded as burn-in before returning samples.

    Returns
    -------
    samples : ndarray, shape (n_post_burn_in * n_walkers, 6)
        Flattened post-burn-in samples; columns 0:3 are electron 1's
        position, columns 3:6 are electron 2's position.
    """
    n_uniform = n_walkers * (6 + 7 * (iterations - 1))
    u = np.random.rand(n_uniform)
    idx = 0

    samples = np.zeros((iterations, n_walkers, 6))

    total_moves = (iterations - 1) * n_walkers
    accepted_moves = 0

    # initial positions: electrons placed near their respective protons
    offset_scale = 0.5
    u_init = u[idx:idx + 6 * n_walkers].reshape(n_walkers, 6)
    idx += 6 * n_walkers
    offsets = offset_scale * (2.0 * u_init - 1.0)

    samples[0, :, 0:3] = q_1 + offsets[:, 0:3]
    samples[0, :, 3:6] = q_2 + offsets[:, 3:6]

    for i in range(1, iterations):
        current = samples[i - 1]

        proposal_steps = step_size * (2 * u[idx:idx + 6 * n_walkers].reshape(n_walkers, 6) - 1)
        idx += 6 * n_walkers
        proposal = current + proposal_steps

        u_accept = u[idx:idx + n_walkers]
        idx += n_walkers

        x_a_c, y_a_c, z_a_c = current[:, 0], current[:, 1], current[:, 2]
        x_b_c, y_b_c, z_b_c = current[:, 3], current[:, 4], current[:, 5]

        x_a_p, y_a_p, z_a_p = proposal[:, 0], proposal[:, 1], proposal[:, 2]
        x_b_p, y_b_p, z_b_p = proposal[:, 3], proposal[:, 4], proposal[:, 5]

        p_current = pdf(x_a_c, y_a_c, z_a_c, x_b_c, y_b_c, z_b_c,
                         theta1, theta2, theta3, q_1, q_2)
        p_prop = pdf(x_a_p, y_a_p, z_a_p, x_b_p, y_b_p, z_b_p,
                      theta1, theta2, theta3, q_1, q_2)

        alpha = np.ones(n_walkers)
        valid = p_current > 0
        alpha[valid] = np.minimum(1.0, p_prop[valid] / p_current[valid])

        accept = u_accept < alpha
        accepted_moves += np.sum(accept)

        samples[i] = np.where(accept[:, None], proposal, current)

    burn_in = max(1, int(0.2 * iterations))
    samples_post = samples[burn_in:, :, :]

    return samples_post.reshape(-1, 6)


def _local_energy(samples, theta1, theta2, theta3, q):
    """
    Local energy E_l for every sample (Eq. 16): two Laplacians (one per
    electron) plus five Coulomb terms (4 electron-nucleus, 1
    electron-electron, plus the fixed nucleus-nucleus repulsion).

    Shared by energy() and energy_with_uncertainty() so the Hamiltonian
    only needs to be defined in one place.
    """
    coords = samples.reshape(-1, 2, 3)
    r1 = coords[:, 0]
    r2 = coords[:, 1]

    q12 = np.linalg.norm(q[0] - q[1])
    h = 10**(-3.5)
    eps = 1e-12  # avoids division spikes if a walker hits a nucleus

    psi_base = psi_h2(r1[:, 0], r1[:, 1], r1[:, 2],
                       r2[:, 0], r2[:, 1], r2[:, 2],
                       theta1, theta2, theta3, q[0], q[1])

    # vectorised central-difference Laplacian, summed over both electrons and xyz
    laplacian = np.zeros_like(psi_base)
    for e_idx in range(2):
        for dim in range(3):
            coords_plus = coords.copy()
            coords_minus = coords.copy()
            coords_plus[:, e_idx, dim] += h
            coords_minus[:, e_idx, dim] -= h

            psi_plus = psi_h2(coords_plus[:, 0, 0], coords_plus[:, 0, 1], coords_plus[:, 0, 2],
                               coords_plus[:, 1, 0], coords_plus[:, 1, 1], coords_plus[:, 1, 2],
                               theta1, theta2, theta3, q[0], q[1])
            psi_minus = psi_h2(coords_minus[:, 0, 0], coords_minus[:, 0, 1], coords_minus[:, 0, 2],
                                coords_minus[:, 1, 0], coords_minus[:, 1, 1], coords_minus[:, 1, 2],
                                theta1, theta2, theta3, q[0], q[1])

            laplacian += psi_plus - 2.0 * psi_base + psi_minus

    laplacian /= h**2

    r1_q1 = np.linalg.norm(r1 - q[0], axis=1) + eps
    r1_q2 = np.linalg.norm(r1 - q[1], axis=1) + eps
    r2_q1 = np.linalg.norm(r2 - q[0], axis=1) + eps
    r2_q2 = np.linalg.norm(r2 - q[1], axis=1) + eps
    r12 = np.linalg.norm(r1 - r2, axis=1) + eps

    potential = (-1.0 / r1_q1 - 1.0 / r1_q2 - 1.0 / r2_q1 - 1.0 / r2_q2
                 + 1.0 / q12 + 1.0 / r12)

    return -(0.5 / psi_base) * laplacian + potential


def energy(samples, theta1, theta2, theta3, q):
    """Monte Carlo estimate of <H> for H2: mean local energy over samples."""
    return np.mean(_local_energy(samples, theta1, theta2, theta3, q))


def energy_with_uncertainty(samples, theta1, theta2, theta3, q, n_blocks=20):
    """
    Monte Carlo estimate of <H> for H2, with a statistical uncertainty
    from block averaging (Eq. 18): the local energy samples are split
    into n_blocks blocks, and the standard error is computed from the
    spread of the block means. This accounts for correlations between
    successive Metropolis samples that would make a naive standard
    error too optimistic.

    Returns
    -------
    mean_E, se_E : float
        Mean energy and its standard error.
    """
    local_E = _local_energy(samples, theta1, theta2, theta3, q)

    n = local_E.size
    n_blocks = int(n_blocks)
    if n_blocks < 2:
        return float(np.mean(local_E)), float("nan")

    block_size = n // n_blocks
    if block_size < 1:
        # too few samples for the requested number of blocks: fall back to a naive SE
        return float(np.mean(local_E)), float(np.std(local_E, ddof=1) / np.sqrt(max(n, 1)))

    trimmed = local_E[:block_size * n_blocks]
    blocks = trimmed.reshape(n_blocks, block_size)
    block_means = blocks.mean(axis=1)

    mean_E = float(block_means.mean())
    se_E = float(block_means.std(ddof=1) / np.sqrt(n_blocks))

    return mean_E, se_E


def monte_carlo_minimisation_2protons(step_size_r, step_size_theta, pdf, iterations, T,
                                       q_1, q_2, q, theta=THETA_INITIAL):
    """
    Simulated annealing over (theta1, theta2, theta3) jointly, for a
    fixed proton separation. Proposals that fall outside (0, 5) for
    theta1/theta2 or [0, 5) for theta3 are rejected outright as
    physically implausible, without spending a Monte Carlo evaluation
    on them.

    Returns
    -------
    theta1, theta2, theta3 : float
        Optimised variational parameters.
    acceptance_pct : float
        Percentage of theta proposals accepted.
    """
    theta1 = theta2 = theta3 = theta

    samples = metropolis_2protons_multi(step_size_r, pdf, 1000, theta1, theta2, theta3, q_1, q_2)
    E = energy(samples, theta1, theta2, theta3, q)

    u = np.random.rand(3 * iterations)
    n = 0
    accepted_theta_number = 0

    for i in range(iterations):
        du1, du2, du3 = 2 * u[3 * i:3 * i + 3] - 1

        theta1_dash = theta1 + step_size_theta * du1
        theta2_dash = theta2 + step_size_theta * du2
        theta3_dash = theta3 + step_size_theta * du3

        if not (0.0 < theta1_dash < 5.0 and
                0.0 < theta2_dash < 5.0 and
                0.0 <= theta3_dash < 5.0):
            continue

        samples_dash = metropolis_2protons_multi(step_size_r, pdf, 500, theta1_dash, theta2_dash,
                                                   theta3_dash, q_1, q_2)
        E_dash = energy(samples_dash, theta1_dash, theta2_dash, theta3_dash, q)

        delta_E = E_dash - E

        if delta_E > 0:
            alpha = np.exp(-delta_E / T)
            accept = np.random.rand() < alpha
        else:
            accept = True

        if accept:
            theta1, theta2, theta3 = theta1_dash, theta2_dash, theta3_dash
            E = E_dash
            accepted_theta_number += 1

        n += 1
        if n == 5:
            T *= 0.5
            n = 0

    return theta1, theta2, theta3, accepted_theta_number / iterations * 100


def plot_h2_pdf_histogram(theta1, theta2, theta3, q_1, q_2, n_walkers=100000,
                           step_size=0.5, bins=600):
    """
    2D histogram of sampled electron positions (x vs z) for H2, with
    proton positions marked (Fig. 5).
    """
    samples = metropolis_2protons_multi(step_size, pdf_xyz, n_walkers, theta1, theta2, theta3, q_1, q_2)

    r1 = samples[:, :3]
    r2 = samples[:, 3:]

    x_vals = np.concatenate([r1[:, 0], r2[:, 0]])
    z_vals = np.concatenate([r1[:, 2], r2[:, 2]])

    plt.figure(figsize=(6, 5))
    plt.hist2d(x_vals, z_vals, bins=bins, density=True, cmap="inferno")
    plt.xlabel("x (a.u.)")
    plt.ylabel("z (a.u.)")
    plt.xlim(-3, 3)
    plt.ylim(-3, 3)
    plt.colorbar(label="Probability Density")
    plt.scatter([q_1[0], q_2[0]], [q_1[2], q_2[2]],
                c="cyan", s=80, marker="x", label="Protons")
    plt.legend(loc="upper right")
    plt.tight_layout()


def morse_total(r, D, a, r0, E_single):
    """Morse potential, project form: D(1 - e^{-a(r - r0)})^2 - D + 2*E_single."""
    return D * (1.0 - np.exp(-a * (r - r0)))**2 - D + 2.0 * E_single


def bond_length(step_size_r, pdf, iterations, T, r_min, r_max, theta_initial, e_single):
    """
    Sweeps the proton separation from r_min to r_max, optimising theta
    and estimating the energy at each separation, then fits the
    resulting energy curve to a Morse potential (Fig. 4).

    Returns
    -------
    D_fit, a_fit, r0_fit : float
        Fitted Morse potential parameters; r0_fit is the predicted
        equilibrium bond length.
    """
    energy_estimates = []
    half_separations = []

    for i in np.arange(r_min, r_max, 0.1):
        q_1 = np.array([i, 0.0, 0.0])
        q_2 = np.array([-i, 0.0, 0.0])
        q = np.array([q_1, q_2])

        theta1, theta2, theta3, _ = monte_carlo_minimisation_2protons(
            step_size_r=step_size_r, step_size_theta=0.3, pdf=pdf, iterations=iterations,
            T=T, q_1=q_1, q_2=q_2, q=q, theta=theta_initial,
        )

        # high-statistics energy estimate at this separation
        samples = metropolis_2protons_multi(step_size_r, pdf, n_walkers=1000, theta1=theta1,
                                              theta2=theta2, theta3=theta3, q_1=q_1, q_2=q_2,
                                              iterations=100)
        energy_estimate = energy(samples, theta1, theta2, theta3, q)

        energy_estimates.append(energy_estimate)
        half_separations.append(i)

    half_separations = np.array(half_separations)
    energies = np.array(energy_estimates)
    r_data = 2.0 * half_separations

    E_min_guess = energies.min()
    r0_guess = r_data[np.argmin(energies)]
    D_guess = energies.max() - E_min_guess
    a_guess = 1.0

    def morse_fit_three(r, D, a, r0):
        return morse_total(r, D, a, r0, e_single)

    params, _ = curve_fit(morse_fit_three, r_data, energies, p0=[D_guess, a_guess, r0_guess])
    D_fit, a_fit, r0_fit = params

    print("Morse fit parameters:")
    print(f"D = {D_fit:.6f}")
    print(f"a = {a_fit:.6f}")
    print(f"r0 = {r0_fit:.6f}  (bond length)")
    print(f"E_min = {morse_total(r0_fit, D_fit, a_fit, r0_fit, e_single):.6f}")

    r_fit = np.linspace(r_data.min(), r_data.max(), 400)
    E_fit = morse_total(r_fit, D_fit, a_fit, r0_fit, e_single)

    plt.figure(figsize=(6, 5))
    plt.scatter(r_data, energies, color="black", s=10, label="MC energies")
    plt.plot(r_fit, E_fit, "r--", label="Morse fit")
    plt.xlabel("Bond length r (a.u.)")
    plt.ylabel("Energy (a.u.)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    return D_fit, a_fit, r0_fit


def main(run_full_sweep=False):
    """
    By default, runs a quick check at a single fixed bond length: optimise
    theta, estimate the energy with uncertainty, and plot the electron
    density histogram.

    Set run_full_sweep=True to additionally run the full bond-length scan
    and Morse fit (Fig. 4) - this is slow, since it repeats the annealing
    optimisation at every separation in the sweep.
    """
    start = time.time()

    q_1 = np.array([0.6611475, 0.0, 0.0])  # position of proton 1
    q_2 = np.array([-0.6611475, 0.0, 0.0])  # position of proton 2
    q = np.array([q_1, q_2])

    theta1, theta2, theta3, _ = monte_carlo_minimisation_2protons(
        0.8, 0.4, pdf_xyz, 200, 1, q_1, q_2, q, THETA_INITIAL,
    )
    print("Optimized theta values for H2 molecule:", theta1, theta2, theta3)

    samples = metropolis_2protons_multi(0.8, pdf_xyz, 10000, theta1, theta2, theta3, q_1, q_2)
    E_mean, E_err = energy_with_uncertainty(samples, theta1, theta2, theta3, q, n_blocks=20)
    print(f"Estimated Energy Expectation Value for H2 molecule (10000 walkers): "
          f"{E_mean:.6f} +/- {E_err:.6f}")

    plot_h2_pdf_histogram(theta1, theta2, theta3, q_1, q_2)

    if run_full_sweep:
        D_fit, a_fit, r0_fit = bond_length(
            step_size_r=1.1, pdf=pdf_xyz, iterations=100, T=0.5,
            r_min=0.2, r_max=2.5, theta_initial=THETA_INITIAL, e_single=E_SINGLE,
        )
        print(f"Bond-length sweep: D={D_fit:.6f}, a={a_fit:.6f}, r0={r0_fit:.6f}")

    end = time.time()
    print(f"Run time: {end - start:.5f} seconds")

    plt.show()


if __name__ == "__main__":
    main(run_full_sweep=False)